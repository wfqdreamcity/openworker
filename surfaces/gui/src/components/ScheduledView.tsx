...
const fmt = (epoch: number | null) =>
  epoch ? new Date(epoch * 1000).toLocaleString(undefined, { dateStyle: "medium", timeStyle: "short" }) : "—";
    ...
    <Shell>
      <div className="flex items-start gap-3">
        <div className="flex-1 min-w-0">
          <PanelHead
            title={t("Automations")}
            sub={t("Recurring tasks OpenWorker runs on a schedule.")}
          />
        </div>
        <button
      ...
      {empty ? (
        !showForm && (
          <div className={CARD + " p-4 text-[12.5px] text-muted"}>
            ...
            {t(", or just ask OpenWorker in a session.")}
          </div>
        )
      ) : (
          ...
          {tasks.map((task) => (
            ...
            >
              ...
              <div className="flex items-center gap-1.5 text-[12px] text-muted">
                ...
                {task.enabled ? task.schedule : t("Paused")} · {t("next")} {fmt(task.next_run)} ·{" "}
                {tn("{n} run", "{n} runs", task.run_count, { n: task.run_count })}
                {task.last_status ? ` · ${t("last")} ${task.last_status}` : ""}
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="sched-detail">
        ...
        {editing ? (
          ...
          <div className="conn-meta">
            ...
            {task.enabled
              ? `${t("Active")} · ${t("next")} ${fmt(task.next_run)}`
              : t("Paused")}{" "}
            · {task.schedule}
          </div>
        )}
            ...
            <div className="sched-run-row">
              <span>
                ...
                {r.artifacts.length > 0 && (
                  <span className="dim"> · {t("{n} file(s)", { n: r.artifacts.length })}</span>
                )}
              </span>
