...
export function AccessSection({
  ...
  const addChannel = async () => {
    ...
    if (!r.ok) {
      setAddErr(r.error || t("Couldn't add that channel."));
      return;
    }
    setAddErr(null);
    setDraft("");
  ...
  const folderPart = projectScoped
    ...
    : roots.length > 0
      ? tn("{n} folder", "{n} folders", roots.length, { n: roots.length })
      : null;
                    ...
                    <div className="flex items-center gap-2 py-1" key={c.connector}>
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        {byName[c.connector]?.channels && (
                          ...
                          >
                            {t("Channels · {n}", { n: channelsOf(c.connector).length })}
...
function ConnectInline({
  ...
  return (
    <div>
      ...
      >
        <Icon name="arrowLeft" size={13} /> {t("Connect {title}", { title: c.title })}
      </button>
      ...
      <p className="text-[10.5px] text-faint mt-2 leading-snug">
        {t("Connecting makes {title} available to all your coworkers — the toggle in this list controls just this session.", {
          title: c.title,
        })}
      </p>
    </div>
  ...
  return (
    <div>
      <button
        ...
        aria-label={t("Back to sources")}
      >
        <Icon name="arrowLeft" size={13} /> {t("{label} channels", { label })}
      </button>
      <div className={`${SEC_H} mb-1.5`}>
        {t("Subscribed channels · {n}", { n: channels.length })}
      </div>
      {channels.length === 0 ? (
        <div className="text-[12px] text-faint py-0.5">
